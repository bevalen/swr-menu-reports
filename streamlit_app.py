...
# Display a message if the user has uploaded a file
if uploaded_file is not None:
    status_text.write("File uploaded successfully.")

# Display a message if the user has not uploaded a file
if uploaded_file is None:
    status_text.write("Please upload a file to continue.")
...
if st.button("Process Data"):
    ...
    status_text.write("Processing data...")
    ...
    hanna_bros_data = save_reports_by_route_to_excel(hanna_bros_sales, f"Hanna Bros. Sales {todays_date}")
    carlyles_sunday_data = save_reports_by_route_to_excel(carlyles_report_sunday, f"Carlyles Report Sunday {todays_date}")
    carlyles_wednesday_data = save_reports_by_route_to_excel(carlyles_report_wednesday, f"Carlyles Report Wednesday {todays_date}")

    # Combine all reports into a single zip file
    zip_buffer = io.BytesIO()

    with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
        zip_file.writestr(f"Hanna_Bros_Sales_{todays_date}.xlsx", hanna_bros_data)
        zip_file.writestr(f"Carlyles_Report_Sunday_{todays_date}.xlsx", carlyles_sunday_data)
        zip_file.writestr(f"Carlyles_Report_Wednesday_{todays_date}.xlsx", carlyles_wednesday_data)
    ...
    status_text.write("Data processed successfully. Ready for download!")